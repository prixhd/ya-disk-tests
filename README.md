...
Также есть параметризованные тесты на проверку создания папок с разными именами
...
- **Изоляция тестов** — каждый тест создаёт ресурсы в `@BeforeEach` и удаляет в `@AfterEach`. Уникальные имена через UUID который исключает конфликты между тестами.
